...
export async function getFiles(folderId, searchQuery = '', pageToken = '') {
  try {
    ...
    let url = `https://www.googleapis.com/drive/v3/files?key=${API_KEY}&q=${encodeURIComponent(query)}&fields=nextPageToken,files(id,name,mimeType,thumbnailLink,webContentLink,iconLink)&pageSize=12&orderBy=folder,name`;
